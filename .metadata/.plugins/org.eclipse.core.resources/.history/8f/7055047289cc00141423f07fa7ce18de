...
public class FSP {
	...
	public void sendMoveByMove(String str)
	{
		...
		double z= (double) stringLength/50;
		...
		while(count<loopCount)
		{
			if(stringLength<50)
				...
				currString=str.substring(count*50,(((count+1)*50)));
			else
				currString=str.substring(count*50,stringLength-1);
			...
			if(Global.realRun==true){
				while(waitingToSend)
				{
					...
					if(checkString.charAt(0)=='O' && checkString.charAt(1)=='K')
						{
							if(count==0){count2=10;System.out.println("Now it is the first fastest part");}
							else count2++;
						}
					if(count2==10)
